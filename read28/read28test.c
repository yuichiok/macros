...
void read28test(){
    ...
    for(int iEntry=0; iEntry<20 ;++iEntry)
    {
        ...
        bool pTbCheck = false;
        //bool muCheck = (dRm > 0.1 || dRmbar > 0.1);
        ...
        cout << "pTm = " << pTm << ", pTmbar = " << pTmbar << endl;
        ...
        if(ZCheck == true && EtabCheck == true && pTbCheck == true){
            ...
            for(int p = 0; p < nJet;++p)
            {
                ...
                pTjet = ptcalc(pxjet[p],pyjet[p]);
                
                cout << "jet" << p << ", dRb = " << dRb << ", dRbbar = " << dRbbar << ", dRm = " << dRm << ", dRmbar = " << dRmbar << ", pTjet = " << pTjet << endl;
                
                if(dRm < 0.1 || dRmbar < 0.1) continue;
            ...
            dRbhist->Fill(tempdR1);
            dRbbarhist->Fill(tempdR2);
            
            cout << "Jet" << tempp1 << " and Jet" << tempp2 << " are picked." <<endl;
